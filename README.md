...
`stock_alert.py` 会定时拉取行情，并在终端打印提醒。
...
```bash
python3 stock_alert.py --once
```

修改检查间隔，例如每 30 秒检查一次：

```bash
python3 stock_alert.py --interval 30
```

默认提醒规则：

- 所有持仓：跌破成本价、当日涨跌幅达到 `±5%`
- 国盾量子：跌破成本 `520.1536` 或当日涨跌幅达到 `±5%`
- *ST大立：跌破 `15.00`、当日跌幅达到 `-3%`、亏损超过 `30%`、ST 风险公告提醒
- 长川科技：跌破成本 `196.9961`、当日跌幅达到 `-5%`、浮盈超过 `30%` 后提示关注盈利保护

> 注意：云端机器不一定会长期保持运行。如果要手机实时推送，后续可以接入 Bark、Server酱、飞书机器人或 Telegram Bot。
