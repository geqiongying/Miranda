...
其中，`*ST大立 002214` 使用持仓风险提醒规则，而不是普通买入规则：

- 跌破 `14.60`：风险线，提醒考虑减仓
- 跌破 `13.30`：强风险线，提醒考虑处理剩余仓位
- 反弹到 `15.10-15.30`：反弹处理观察区，若站不稳提醒考虑减仓
- 站上 `16.50`：转强观察线，提醒复核是否继续持有

脚本只做提醒，不会也不能自动下单。实际买卖仍需你打开券商软件手动确认。
...
配置 PushPlus token 后发送提醒；默认会同时检查国投中鲁、飞龙股份和 *ST大立，任意一只触发都会推送：
